...
class ChangeBackgroundColor(Node):
    ...
    def send_request(self, r, g, b):
        ...
        # if self.future.result() is not None:
            # if self.future.result().successful:
                # self.get_logger().info('Background color set successfully')
            # else:
                # self.get_logger().error('Failed to set background color: ' + self.future.result().reason)
        # else:
            # self.get_logger().error('Service call failed')
        # self.future = None # Reset future to None
